...
const char* password = "giankasa0610";
...
void setup() {
  ...
  IPAddress ip(192,168,118,138);
  IPAddress gateway(192,168,118,2);
  ...
  if(llave == "pwm"){
    ...
    switch (llave[1]){
      case 'f':
        ...
        shiftOut(ab, clk, LSBFIRST, B00000011)
        break;
      case 'b':
        Serial.println("Luces traseras");
        shiftOut(ab, clk, LSBFIRST, B00001100)
        break;
      case 'l':
        Serial.println("Luces izquierda");
        shiftOut(ab, clk, LSBFIRST, data)
        if (derechaEncendida) {
        shiftOut(ab, clk, LSBFIRST, B00111111); // Apagar la luz direccional derecha (bits 7 y 8)
        derechaEncendida = false;
      } else {
        shiftOut(ab, clk, LSBFIRST, B11000000) // Encender la luz direccional derecha (bits 7 y 8)
        derechaEncendida = true;
      }
        break;
        break;
      case 'r':
      Serial.println("Luces derechas (parpadeando)");
      static bool derechaEncendida = false; // Variable para controlar el estado de las luces derechas
      if (derechaEncendida) {
        shiftOut(ab, clk, LSBFIRST, B00111111); // Apagar la luz direccional derecha (bits 7 y 8)
        derechaEncendida = false;
      } else {
